...
int appoint(sqlite3 *db, int venueId, const char *date, const char *start, const char *end, uint32_t appointmentId) {
    ...
    while (sqlite3_step(stmt0) == SQLITE_ROW) {
        const char *curStart = sqlite3_column_text(stmt0, 0);
        const char *curEnd = sqlite3_column_text(stmt0, 1);
    ...
    while (sqlite3_step(stmt2) == SQLITE_ROW) {
        const char *client_addr = sqlite3_column_text(stmt2, 0);
...
static int adjust_time(sqlite3 *db, const int venueId, const char *date, const char *startTime, const char *endTime, uint32_t appointmentId, double offset) { 
    ...
    while (sqlite3_step(stmt0) == SQLITE_ROW) {
        ...
        const char *curStart = sqlite3_column_text(stmt0, 1);
        const char *curEnd = sqlite3_column_text(stmt0, 2);
...
int alter(sqlite3 *db, uint32_t appointmentId, double offset) {   
    ...
    if ((rc = sqlite3_step(stmt0)) == SQLITE_ROW) {
	   ...
	   const char *startTime = sqlite3_column_text(stmt0, 2);
	   const char *endTime = sqlite3_column_text(stmt0, 3);
    ...
    while (sqlite3_step(stmt1) == SQLITE_ROW) {
        const char *client_addr = sqlite3_column_text(stmt1, 0);
        printf("HOOK! send to %s\n", client_addr);
        // sendto client for new info
        }
    }
...
//    appoint(db, 1, "2025-09-06", "10:00", "16:00", appointmentId);
//    appoint(db, 1, "2025-09-05", "11:00", "13:00", appointmentId);
//    alter(db, 1349075284, -3.0);
//    alter(db, 3944530291, 1);
    //delete(db, 5);
    hook(db, "192.168.8.129", 1, parseTime("2025-09-05", "11:00"), 120); 
    sqlite3_close(db);
    return 0;
}
